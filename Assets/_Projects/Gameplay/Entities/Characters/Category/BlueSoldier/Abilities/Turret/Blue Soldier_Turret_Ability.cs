    ...
    public class BlueSoldier_Turret_Ability : CharacterAbility
    {
        ...
        private void AttackTargetHandle()
        {
            ...
            if (_attackCooldown.IsComplete)
            {
                ...
                this.SpawnVFX(shootPosition, hit.collider != null ? hit.point : shootPosition + direction.normalized * _distance);

                if (hit.collider == null) return;
                if (hit.transform != _target.transform) return;

                CombatController.Instance.DamageDealing(_target, _damage);
            }
        }
