...
{
    public class SentinelTurret_Machine : Machine, IMachineFireable
    {
        [Header("Detection")]
        ...
        private void AttackTargetHandle()
        {
            ...
            if (_attackCooldown.IsComplete)
            {
                ...
                this.Fire(shootPosition, direction);
            }
        }
        ...
        public void Fire(Vector2 position, Vector2 direction)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(position, direction.normalized, viewRadius, seeLayer);

            this.SpawnVFX(position, hit.collider != null ? hit.point : position + direction.normalized * viewRadius);
            OnFired?.Invoke(position, direction);

            if (hit.collider == null) return;
            if (!hit.transform.TryGetComponent(out Entity entity)) return;

            CombatController.Instance.DamageDealing(new DamageContainer(this, entity)
            {
                Damage = _damage
            });
            EffectController.Instance.AddEffect("Slow", this, entity, new EffectData()
            {
                Duration = 3f,
                Strength = 0.4f,
            });
